...
# Create a PGM object with professional styling
pgm = daft.PGM(dpi=150, alternate_style="outer")

# Add Lemons node (independent variable)
pgm.add_node("lemons", "Lemons" + r" $(X)$", 1, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#FFE66D',  # Warm yellow for lemons
                 'edgecolor': '#F4A259',   # Orange border
                 'linewidth': 2.5,
                 'alpha': 0.9,
             })

# Add Scurvy Prevention node (dependent variable)
pgm.add_node("scurvy", "Scurvy\nPrevention" + r" $(Y)$", 3.5, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#06D6A0',  # Teal for health/prevention
                 'edgecolor': '#118AB2',  # Deeper blue border
                 'linewidth': 2.5,
                 'alpha': 0.9,
             })

# Add edge showing causal relationship
pgm.add_edge("lemons", "scurvy", plot_params={'linewidth': 2.5, 'color': '#073B4C'})

# Render the DAG
pgm.render()
plt.title("1747 Understanding: Lemons Prevent Scurvy", fontsize=14, fontweight='bold', pad=15)
plt.tight_layout()
plt.show()
```
...
#| echo: true
#| fig-cap: "DAG 2: The Misguided Belief - Acid kills bacteria to prevent scurvy (wrong understanding)"
...
# Add Acid node (independent variable) - using red/warning colors - TOP
pgm.add_node("acid", "Acid" + r" $(X)$", 2.5, 3, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#FFB6B9',  # Light coral pink (warning color)
                 'edgecolor': '#E63946',  # Red border (error/wrong)
                 'linewidth': 2.5,
                 'alpha': 0.85,
             })

# Add Bacteria Death node (mediator variable) - using orange/amber colors - MIDDLE
pgm.add_node("bacteria", "Bacteria\nDeath" + r" $(M)$", 2.5, 2, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#FFDAB9',  # Peach puff
                 'edgecolor': '#FF6B35',  # Orange border
                 'linewidth': 2.5,
                 'alpha': 0.85,
             })

# Add Scurvy Prevention node (dependent variable) - still using muted colors - BOTTOM
pgm.add_node("scurvy", "Scurvy\nPrevention" + r" $(Y)$", 2.5, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#C9ADA7',  # Muted mauve (false conclusion)
                 'edgecolor': '#9A8C98',  # Gray-purple border
                 'linewidth': 2.5,
                 'alpha': 0.85,
             })

# Add edges showing the incorrect causal chain
pgm.add_edge("acid", "bacteria", plot_params={'linewidth': 2.5, 'color': '#E63946'})
pgm.add_edge("bacteria", "scurvy", plot_params={'linewidth': 2.5, 'color': '#E63946'})

# Render the DAG
pgm.render()
plt.title("Misguided Belief: Acid → Bacteria Death → Scurvy Prevention (WRONG!)", 
...
# Add Vitamin C node (independent variable) - using scientific blue-green colors
pgm.add_node("vitamin_c", "Vitamin C\nIntake" + r" $(X)$", 1, 1, aspect=3, scale=1.3,
             plot_params={
                 'facecolor': '#A8DADC',  # Powder blue (scientific/medical)
                 'edgecolor': '#457B9D',  # Steel blue border
                 'linewidth': 3,
                 'alpha': 0.95,
             })

# Add Scurvy Prevention node (dependent variable) - using success green colors
pgm.add_node("scurvy", "Scurvy\nPrevention" + r" $(Y)$", 3.8, 1, aspect=3, scale=1.3,
             plot_params={
                 'facecolor': '#06D6A0',  # Teal green (health/success)
                 'edgecolor': '#1D3557',  # Deep navy border
                 'linewidth': 3,
                 'alpha': 0.95,
             })

# Add edge showing the correct causal relationship
pgm.add_edge("vitamin_c", "scurvy", plot_params={'linewidth': 3, 'color': '#1D3557', 'alpha': 0.9})

# Render the DAG
pgm.render()
plt.title("1928 Understanding: Vitamin C Prevents Scurvy (CORRECT!)", 
          fontsize=14, fontweight='bold', pad=15, color='#1D3557')
plt.tight_layout()
plt.show()
```
